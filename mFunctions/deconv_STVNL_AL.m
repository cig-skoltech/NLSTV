function [x,fun_val,residual,ISNR]=deconv_STVNL_AL(y,h,lambda,W,C,varargin)

%Deconvolution with the Non Local Structure Tensor Total Variation (NL-STV) 
%regularizer under convex constraints using Augmented Lagrangian.
%
%  argmin  0.5*||y-Hx||^2+ lambda*||z1||_(1,p) + i_C(z2),
%  x,z1,z2
% s.t z1=D(x)
%     z2=x
%
% H : convolution operator (assumed to be circulant);
...
if p < 1
  error('deconv_STVNL_AL: Invalid Schatten norm order.');
end
...
ConvOp=@(x)real(ifft2(H.*fft2(x))); % H*x
AdjConvOp=@(x)real(ifft2(conj(H).*fft2(x))); % H^T*x
DConvOp=@(x)real(ifft2(abs(H).^2.*fft2(x)))/alpha; % (1/alpha)*(H^T*H*x)
count=0;
W=sqrt(W);
...
yT_over_alpha=AdjConvOp(y)/alpha;

if verbose
  fprintf('*********************************************************\n');
  fprintf('**        Deconvolution with NL-STV Regularizer        **\n');
  fprintf('*********************************************************\n');
...
for i=1:maxiter
 ...
 if pflag
   ...
   %v1=z1-s1
   %v2=z2-s2
   b=yT_over_alpha+AdjJacobianOp2D_NL(z1-s1,W,idx,n,I,bc)+(z2-s2);
   A=@(x)JTJOp2D_NL(x,W,idx,n,I,bc)+DConvOp(x)+x;
   ...
   %v1=z1-s1
   b=yT_over_alpha+AdjJacobianOp2D_NL(z1-s1,W,idx,n,I,bc);
   A=@(x)JTJOp2D_NL(x,W,idx,n,I,bc)+DconvOp(x);
   xnew=CG_solver(A,b,x,cg_iter,cg_tol);
   tmp1=JacobianOp2D_NL(xnew,W,C,bc)-z1;
   s1=s1+tmp1;
 end
  ...
  if verbose
    if ~isempty(img)
      ...
      fun_val(i)=cost(y,ConvOp,x,W,C,lambda,bc,p);
...
function [Q,NLSTVnorm]=cost(y,ConvOp,f,W,C,lambda,bc,p)
...
tmp=ConvOp(f);
Q=0.5*norm(y(:)-tmp(:),2)^2+lambda*NLSTVnorm;
